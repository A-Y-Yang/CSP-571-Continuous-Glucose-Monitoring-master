...
periods_train <- 12*24*2 # 2 days
periods_test  <- 12*3 # 2 hours
skip_span     <- 12 * 24
...
sample_predictions_lstm_tbl <- rolling_origin_resamples %>%
     mutate(predict = map(splits, predict_keras_lstm, epochs = 250))
```
...
```{r}
future_cgm_tbl <- predict_keras_lstm_future(cgm_data, epochs = 300)
```

```{r}
future_cgm_tbl %>%
    filter_time("2019-03-01 00:00" ~ "end") %>%
